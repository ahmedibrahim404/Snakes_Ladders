...
void CardEleven::ReadCardParameters(Grid* pGrid)
{
	...
	pOut->PrintMessage("New Card 11: Enter fees the passing player should pay ...");
...
void CardEleven::Apply(Grid* pGrid, Player* pPlayer)
{
	...
	if (owner == nullptr) { // if station is not bought
		...
		if (chose == 1) { // if option is to buy
			...
			pOut->PrintMessage("Player " + to_string(pPlayer->getPlayerNumber()) + " now owns stations 11");
		}
